		...
		async (request, reply) => {
			...
			const chunks = await db
				.select({
					...
					similarity: sql<number>`1 - (${schema.audioChunks.embeddings} <=> ${embeddingsAsString}::vector)`, // será retornado um número
				})
				...
				) //ordenando pelos valores mais proximos de similaridade
				.limit(3);
